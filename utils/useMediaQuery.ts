 ...
 */
export function useMediaQuery(query: string | string[], defaultValues?: boolean | boolean[]): boolean[] {
	...
	useSafeLayoutEffect(() => {
		...
		return () => {
			mql.forEach((mql) => {
				if (typeof mql.removeListener === 'function') mql.removeListener(handler);
